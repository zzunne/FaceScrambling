...
void main() {
	...
	string inputFilename = "../GIRL/GIRL.gray"; 
	string outputFilename = "../GIRL/girl_scrambled.gray";

	loadImage(inputFilename);				// 이미지 불러오기
	encoding(input_data);					// encoding(scrambling)
...
void encoding(char** original) {
	...
	for (int i = 0; i<ROI_ROW; i++)
		for (int j = 0; j < ROI_COL; j++) {
			...
			temp_data[i][j] = input_data[i + ROI_BEGIN_ROW][j + ROI_BEGIN_COL];
		}
	...
	for (int i = 0; i < ROI_ROW*ROI_COL; i++) 
		roi_data[i / ROI_ROW][i % ROI_COL] = temp_data[key_arr[i] / ROI_ROW][key_arr[i] % ROI_COL];
	
	// temp_data와 key_arr 메모리 해제
	for (int i = 0; i < ROI_ROW; i++) delete temp_data[i];
	delete key_arr, temp_data;

	// input_data(원본)에 scramble된 얼굴 추가
	for (int i = 0; i < ROI_ROW; i++)
		for (int j = 0; j < ROI_COL; j++) 
			input_data[i + ROI_BEGIN_ROW][j + ROI_BEGIN_COL] = roi_data[i][j];
...
void saveImage(char** encoded, string filename) {
	// 파일 출력 스트림 변수 선언
